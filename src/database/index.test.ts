...
describe("Test the dictionary", () => {
    ...
    test('Test addIndex', () => {
        ...
        const index = indexline.parse('test_christmas_tree n 5 2 @ #m 5 0 12787364 12738599 11621547 11621281 03026626  ')
    ...
    test('Test IndexOffsetSearch', () => {
        ...
        expect(result.get(12787364)!.map(item => item.lemma).join(',')).toContain('christmas_tree')
        expect(result.get(12787364)!.map(item => item.offsets).join(',')).toContain('12787364')
    ...
    test('Test DataLemmaSearch', () => {
        ...
        expect(result.get('christmas_tree')!.map(item => item.words).join(',')).toContain('christmas_tree')
        expect(result.get('christmas_tree')!.map(item => item.offset).join(',')).toContain('12787364')
        ...
        expect(result.get('christmas_tree')!.map(item => item.words).join(',')).toContain('christmas_tree')
        expect(result.get('christmas_tree')!.map(item => item.offset).join(',')).toContain('12787364')
        expect(result.get('preposterous')!.map(item => item.words).join(',')).toContain('preposterous')
        expect(result.get('preposterous')!.map(item => item.offset).join(',')).toContain('2570643')
    })
