...
class Dictionary {
  ...
  wordsUsingAllCharactersFrom(query: string, ignorePhrases = true) {
    ...
    output = this.database.index
      .filter((item) => {
        ...
        if (
          ignorePhrases &&
          (lemmaSplit.includes("_") || lemmaSplit.includes("-"))
        ) {
          return false;
        }
  ...
  wordsWithCharsIn(query: string, priorityCharacters = "") {
    const matchingWords = this.database.index
      .filter((item) => Dictionary.hasAllCharsIn(query, item.lemma))
      .map((item) => item.lemma)
      .sort((a, b) => {
        let diff = 0;
        if (priorityCharacters.length > 0) {
          const aPriority = Dictionary.hasAllCharsIn(a, priorityCharacters)
            ? 10
            : 0;
          const bPriority = Dictionary.hasAllCharsIn(b, priorityCharacters)
            ? 10
            : 0;
