...
describe("Test the dictionary", () => {
    ...
    test('Test searchWord', () => {
        ...
        expect(result.get('coaxing')!.get("noun")!.offsetData.map(item => item.words).join(',')).toContain('coaxing')
        expect(result.get('coaxing')!.get("noun")!.offsetData.map(item => item.glossary).join(',')).toContain('flattery designed to gain')
        ...
        expect(result.get('yet')!.get("adverb")!.offsetData.map(item => item.words).join(',')).toContain('yet')
        expect(result.get('yet')!.get("adverb")!.offsetData.map(item => item.glossary).join(',')).toContain('largest drug bust yet')
    })
    ...
    test('Test searchSimple', () => {
        ...
        expect(result.get('drink')!.get("noun")!.meaning).toBe('a single serving of a beverage')
    })
