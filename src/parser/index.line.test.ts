...
describe("Test parsing an index line", () => {
    test("Parse an index line", () => {
        let item = new IndexLine().parse("  7 database and its documentation for any purpose and without fee or  ")
        expect(item.isComment).toBe(true)

        item = new IndexLine().parse("advanced a 8 1 & 8 8 01840121 00819526 01876261 01211296 01208738 00816839 00412171 00198383")
        ...
        expect(item.pointers).toEqual([{'offset': 0, pos: 'adjective', symbol: 'Similar to'}])
        expect(item.senseCount).toBe(8)
        expect(item.tagSenseCount).toBe(8)
        expect(item.offsets).toEqual([1840121, 819526, 1876261, 1211296, 1208738, 816839, 412171, 198383])
        ...
        expect(item.pointerCount).toBe(1)
        expect(item.pointers).toEqual([{"offset": 0, "pos": "adjective", "symbol": "Pertainym (pertains to noun)"}])
        ...
        item = new IndexLine().parse("a_posteriori a 2 3 ! & ^ 2 0 00139126 00859350  ")
        ...
        item = new IndexLine().parse("armed a 3 3 ! & ; 3 1 00142407 00146210 00144877  ")
